...
if [ $db_enable = yes ];then
    if [ "$multistation" == no ];then
        /usr/local/mysql/bin/mysqldump --opt -u$DB_USER -p$DB_PASS -h$DB_HOST $DB_NAME  > $backup_file/$backuptime.sql
    ...
    if [ "$multistation" == yes ] && [ ! -z "$multisitenumber" ];then
        for((msnb=0;msnb<="$multisitenumber";msnb++));do
            mysqldump --opt -u$DB_USER -p$DB_PASS -h$DB_HOST "${multdblist["$msnb"]}"  > "${backup_filelist["$msnb"]}"/$backuptime.sql
        done
    fi
else
    echo "数据库备份关闭"
...
#检查./.cos.conf环境开始
    if [ -f ~/.cos.conf ];then
        echo "cos.conf检测完毕"
		rm -rf ~/.cos.conf
        coscmd config -a "$txaccess_id" -s "$txsecret_key" -u "$txappid" -b "$txbucketname" -r "$txregion"		
    else
...
#腾讯云删除开始
    if [ ! -z "$rmdate" ];then 
        if [ "$multistation" == no ];then
#        coscmd delete "$rmbackuptime".tar.gz  
    ...
    if [ "$multistation" == yes ] && [ ! -z "$multisitenumber" ];then
        for((msnb=0;msnb<="$multisitenumber";msnb++));do
#            coscmd delete  "$txbucketname"/"${multlist["$msnb"]}"/"$rmbackuptime".tar.gz >/dev/null
             coscmd delete  "${multlist["$msnb"]}"/"$rmbackuptime".tar.gz >/dev/null
            if [ "$?" -eq 0 ];then
